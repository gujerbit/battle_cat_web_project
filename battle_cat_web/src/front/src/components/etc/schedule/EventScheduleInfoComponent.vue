<template>
  <section>
    <div id="schedule">
      <p class="schedule-title">이벤트 일정</p>
      <div class="title">
        <p>내용</p>
        <p>시작</p>
        <p>종료 시간</p>
...
import { getEventSchedule } from '../../../js/etc/schedule/schedule.js';
...
export default {
  setup() {
    ...
    const contentUpdate = () => { //현재 번호에 해당하는 내용 업데이트
      schedule.value.currentData = pagination(getEventSchedule(proxy.store), pageInfo.value.currentPage, pageInfo.value.divisionPage); //현재 번호에 해당하는 내용 페이지네이션
      pageInfo.value.totalPage = pageDivision(getEventSchedule(proxy.store), pageInfo.value.divisionPage); //현재 요일 전체 데이터를 기반으로 총 페이지 수 정하기
    };

    onBeforeMount(() => { //dom에 부착되었을 때
      const loadData = setInterval(() => {
        if(getEventSchedule(proxy.store) !== undefined) {
          schedule.value.data = getEventSchedule(proxy.store);
          contentUpdate();
