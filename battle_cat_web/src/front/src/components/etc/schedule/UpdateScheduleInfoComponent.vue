<template>
  <section>
    <div id="schedule">
      <p class="schedule-title">업데이트 현황</p>
      <div class="title">
        <p>버전</p>
        <p>업데이트 날짜</p>
      </div>
      <router-link :to="`/update_specific/${value.version}`" class="info" v-for="(value, index) in schedule.currentData" :key="value">
        <p :class="index === 0 ? 'select' : ''">{{value.version}}</p>
        <p :class="index === 0 ? 'select' : ''">{{value.update_date}}</p>
      </router-link>
    </div>
    <div class="pages">
...
import { getUpdateSchedule } from '../../../js/etc/schedule/schedule.js';
...
export default {
  setup() {
    ...
    const contentUpdate = () => { //현재 번호에 해당하는 내용 업데이트
      schedule.value.currentData = pagination(getUpdateSchedule(proxy.store), pageInfo.value.currentPage, pageInfo.value.divisionPage); //현재 번호에 해당하는 내용 페이지네이션
      pageInfo.value.totalPage = pageDivision(getUpdateSchedule(proxy.store), pageInfo.value.divisionPage); //현재 요일 전체 데이터를 기반으로 총 페이지 수 정하기
    };

    onBeforeMount(() => { //dom에 부착되었을 때
      const loadData = setInterval(() => {
        if(getUpdateSchedule(proxy.store) !== undefined) {
          schedule.value.data = getUpdateSchedule(proxy.store);
          contentUpdate();
