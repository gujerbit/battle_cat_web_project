...
export default {
  setup() {
    ...
    onBeforeMount(() => {
      if(window.sessionStorage.getItem('passwordChangeInfo') != null) {
        result.value.email = JSON.parse(window.sessionStorage.getItem('passwordChangeInfo')).email;
        result.value.registerCode = JSON.parse(window.sessionStorage.getItem('passwordChangeInfo')).passwordChangeCode;
        result.value.expireTime = JSON.parse(window.sessionStorage.getItem('passwordChangeInfo')).expireTime;
      }
    });
...
#find-email p, #find-password p {
  font-size: 5rem;
  color: #ffc038;
  text-shadow: -1px 0 #000000, 0 1px #000000, 1px 0 #000000, 0 -1px #000000;
}

#find-email:hover, #find-password:hover {
  transform: scale(90%);
}

.email, .password, .password-change-code, .password-change {
  width: 90%;
  height: 100%;
  margin: 0 auto;
}

.email, .password, .result, .password-change-code, .password-change {
  display: flex;
  justify-content: center;
  align-items: center;
}
...
.email > .content, .result > .content, .password-change-code > .content {
  width: 35%;
  height: 25%;
  border: 2px solid #ffc038;
  border-radius: 15px;
...
.password-change > .content {
  ...
  border-radius: 15px;
  display: grid;
  grid-template-rows: 20% 5% 25% 5% 25% 20%;
  justify-items: center;
  align-items: center;
}

.email > .content {
  grid-template-rows: 25% 5% 40% 30%;
}
...
.result-field :nth-child(n):hover {
  transform: scale(95%);
}
...
.main-page:hover, .login-page:hover {
  opacity: 1;
}
